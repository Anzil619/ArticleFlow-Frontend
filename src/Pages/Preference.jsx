...
import { CreateBulkPreference, EditProfileDetails, GetCategory } from "../Services/Services";
  ...
  {category?.map((e, index) => (
    <button
      ...
      className={`px-10 py-4 rounded-full cursor-pointer border-0 bg-${
        selectedCategories.includes(e.id) ? 'blue-gray-400' : 'white'
      } shadow-md uppercase text-sm transition duration-500 ease-in-out transform hover:scale-105 hover:shadow-lg ${
        selectedCategories.includes(e.id) ? 'text-white' : 'text-black'
      }`}
      ...
      {e.category}
          ...
          <div className="flex justify-end mt-10">
            <button
              ...
              className="px-10 text-white py-4 rounded-full cursor-pointer border-0 bg-black shadow-md uppercase text-sm transition duration-500 ease-in-out transform hover:scale-105 hover:shadow-lg hover:bg-indigo-600 hover:text-white active:bg-indigo-700 active:shadow-none active:translate-y-1"
            >
              Continue
            </button>
          </div>
